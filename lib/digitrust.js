...
async function call(path, options = {}) {
  ...
  try {
    ...
    // Network error or timeout — we genuinely don't know whether
    // DigiTrust received/processed this request. Callers that care
    // (fulfillOrder) check `err.ambiguous` to avoid treating this the
    // same as a clean "no, that failed" response from the API.
    const err = new Error(
      e.name === "AbortError" ? "DigiTrust request timed out" : e.message
    );
    err.ambiguous = true;
    throw err;
  } finally {
  ...
  if (!res.ok || data.success === false) {
    // We got a real response from DigiTrust saying no — safe to treat
    // as a clean failure (nothing was purchased).
