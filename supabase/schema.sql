...
-- Postgres (which includes Supabase's anon/authenticated roles).
-- Since these bypass RLS by design, that default would let any
-- logged-in user call them directly via supabase.rpc(...) from the
-- browser and move balance around with no verification at all. Our
-- server code only ever calls these via the service_role key, so
-- locking this down breaks nothing legitimate.
